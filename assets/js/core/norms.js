...
export const NORMS = {
  ...
  sampleNote:
    '百分位参照的是这样一群人：浙江省 6 所高校有恋爱经历的大学生，有效样本 547 人（男 263 / 女 284），年龄 18–24 岁。' +
    '他们做的是李同归、加藤和生（2006）修订的 ECR 中文版——36 题、7 点计分（发表于《心理学报》38(3): 399-406）。',
  ...
  spreadNote:
    '不同研究之间的正常波动：普通大学生样本的依恋焦虑在 3.54–3.77 之间，依恋回避在 3.14–3.47 之间，相差约 ±0.2。' +
    '特殊群体会明显偏离——比如有童年留守经历的那组，回避维度到了 4.16。' +
    '所以「你在人群里排第几」这件事本身有波动，不适合当成一把精确的尺子。',

  itemSetCaveat:
    '最后必须说明：本站的 36 道题是我们按同一套思路重新写的，不是 ECR 原题。' +
    '所以百分位表示「对照上面那群人，你大概在什么位置」，而不是在本站题目上重新测出来的排名；' +
    '类型判定用的分界线是量表正中间的 4.0，不是任何一组样本的中位数。两者的区别已在结果页分别标注。',
